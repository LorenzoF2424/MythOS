...
setupPagingTables:
    ...
    mov edi, PDPT_ADDR
    mov eax, 0x12003    ; address of the first PD
    mov [edi], eax      ; PDPT[0] -> 0-1GB
    add eax, 0x1000
    mov [edi + 8], eax  ; PDPT[1] -> 1-2GB
    add eax, 0x1000
    mov [edi + 16], eax ; PDPT[2] -> 2-3GB
    add eax, 0x1000
    mov [edi + 24], eax ; PDPT[3] -> 3-4GB

    mov edi, PD_ADDR        ; Inizia a scrivere dalla prima PD (0x12000)
    mov eax, 0x00000083     ; Present(1) + Write(2) + PS(0x80) = 2MB Page
    mov ecx, 2048           ; 512 entrate * 4 tabelle = 4GB mappati
...
switchToLongMode:
    ...
    mov eax, cr0
    and ax, 0xFFFB      ; Pulisce bit 2 (EM - Emulation)
    or ax, 0x0002       ; Imposta bit 1 (MP - Monitor Coprocessor)
    mov cr0, eax
    
    mov eax, cr4
    or ax, 3 << 9       ; Imposta bit 9 (OSFXSR) e bit 10 (OSXMMEXCPT)
    mov cr4, eax
