...
int strncmp(const char* s1, const char* s2, size_t n) {
    // Early return: comparing zero characters always yields equality
    if (n == 0) return 0;

    // Early return: armor against null pointers to prevent kernel panics
    if (s1 == nullptr && s2 == nullptr) return 0;
    if (s1 == nullptr) return -1;
    if (s2 == nullptr) return 1;

    for (size_t i = 0; i < n; i++) {
        // Early return: a difference is found
        if (s1[i] != s2[i]) {
            // Cast to unsigned char is standard practice in C to handle extended ASCII safely
            return (unsigned char)s1[i] - (unsigned char)s2[i];
        }
        
        // Early return: both strings terminated perfectly at the same time
        if (s1[i] == '\0') {
            return 0;
        }
    }

    // All 'n' characters checked and they matched
    return 0;
}
...
extern "C" void* memcpy(void* dest, const void* src, size_t n) {
    // Early return: nothing to copy or pointers point to the same address
    if (n == 0 || dest == src) return dest;

    // Early return: guard against null pointers to prevent kernel panics
    if (!dest || !src) return nullptr;

    // Cast pointers to 64-bit for fast block copying
    uint64_t* d64 = (uint64_t*)dest;
    const uint64_t* s64 = (const uint64_t*)src;

    // Calculate the number of 8-byte chunks and the remaining bytes
    size_t chunks = n / 8;
    size_t leftovers = n % 8;

    // Fast copy for 64-bit chunks
    for (size_t i = 0; i < chunks; i++) {
        d64[i] = s64[i];
    }

    // Cast the remaining addresses to 8-bit pointers
    uint8_t* d8 = (uint8_t*)(d64 + chunks);
    const uint8_t* s8 = (const uint8_t*)(s64 + chunks);

    // Copy the remaining individual bytes
